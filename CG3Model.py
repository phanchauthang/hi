...
############################
        ...
        p_e_xy_1 = -tf.reduce_mean(tf.math.log(tf.nn.sigmoid(self.p_e_yy_w_contra(tf.concat([y_ei_gcn, y_ej_hgcn], axis = 1))))) 
        p_e_xy_2 = -tf.reduce_mean(tf.math.log(tf.nn.sigmoid(self.p_e_yy_w_contra(tf.concat([y_ei_hgcn, y_ej_gcn], axis = 1)))))
...
#################################        
    ...
    def ContrastiveLoss(self):
        ...
        diag_cos = tf.linalg.diag_part(cos_dist) 
        ...
        self.loss += -hp1 * tf.reduce_mean(tf.math.log(pos_neg3))
        ...
        self.loss += -hp1 * tf.reduce_mean(tf.math.log (pos_neg_sup_3))    #0.9
